...
function refreshGoods(goods){
    ...
    $(goods).each(function(i,item){
        var tr ="<tr ondblclick='ondbclickGoods("+item.id+",&quot;"+item.name+"&quot;"+","+item.storage+","+item.sales+","+item.sellPrice+")'><td>"+ item.name +"</td><td>" +item.birthplace+"</td><td>"+item.specification+
            "</td><td>"+item.packaging+"</td><td>"+item.sellPrice+"</td>"+"</tr>";
        tBody += tr;
...
function addOrUpdateGoods() {
    ...
    function ondbclickGoods(id,name,storage,sales,sellPrice){
        $("#add").modal('hide');
        $("#goodsId").val(id);
        $("#goodsName").val(name);
        $("#storage").val(storage);
